...
#        'fsym': crypto_id,
#        'tsym': 'USD',
#        'limit': min(limit, hours) - 1  # Adjust limit for the last request
#    }
#
#    # Set hours amount to fetch at the end of pagination
#    target_hours = hours 
#
#    while hours > 0:
...
#            print(f"Request to API with params: {params}")
#            print(f"Response status: {response.status_code}, Data length: {len(data)}")
#            all_data.append(df)
...
#            params['limit'] = min(limit, hours) - 1  # Adjust limit for the next request
#        else:
...
#    
#    # Trim to the exact number of required hours
#    logger.info(f"1 call: {full_df.head()}")
#    logger.info(f"target hours: {target_hours}")
#    #full_df = full_df.iloc[:target_hours]
#
#    if len(full_df) > target_hours:
#        full_df = full_df.iloc[-target_hours:]
#
#    logger.info(f"2 call: {full_df.head()}")
#
#    # Set index and keep 'date' as a column
#    full_df.set_index('date', inplace=True)
#    full_df.reset_index(drop=False, inplace=True)
#
#    logger.info(f"3 call: {full_df.head()}")
#
