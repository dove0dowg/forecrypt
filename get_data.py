...
#    :param api_key: API key for authentication (optional).
#    :return: A pandas DataFrame with columns ['date', 'price'] and a DatetimeIndex (UTC, hourly frequency).
#    """
#    url = "https://min-api.cryptocompare.com/data/v2/histohour"
#    headers = {"Authorization": f"Apikey {api_key}"} if api_key else {}
#    all_data = []
...
#
#            df = pd.DataFrame(data)
#            df['date'] = pd.to_datetime(df['time'], unit='s')  # naive
#            all_data.append(df)
#
#            hours -= len(df)
...
#
#    # Concatenate all partial DataFrames and set the index to 'date'
#    full_df = pd.concat(all_data).sort_index()
#    full_df.set_index('date', inplace=True)
#    print(df.index.duplicated().sum())
#    print(full_df)
#    #full_df = full_df.asfreq('h')  # Set DatetimeIndex to hourly frequency
